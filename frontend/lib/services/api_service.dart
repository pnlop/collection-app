...
class ApiService {
  ...
  Future<List<CollectionModel>> getCollections() async {
    ...
    if (response.statusCode == 200) {
      ...
      return collections.map((json) => CollectionModel.fromJson(json)).toList();
    } else {
  ...
  Future<List<CardModel>> searchCards(String query) async {
    ...
    if (response.statusCode == 200) {
      final Map<String, dynamic> data = json.decode(response.body);
      final List<dynamic> cards = data['searchResults'];
  ...
  Future<CardModel> updateCard(String collectionName, CardModel card) async {
    final response = await http.put(
      Uri.parse('$baseUrl/api/collections/$collectionName/cards/${card.name}'),
      headers: {'Content-Type': 'application/json'},
      body: json.encode(card.toJson()),
    );
